...
queries = [
    ...
    },
    {
        ...
        "expected_response": {
            "code":"result = df[df.workflow_id == df.loc[df.started_at.idxmax(), 'workflow_id']].groupby('activity_id').agg({col: 'mean' for col in df.columns if col.startswith('generated.')})",
    ...
    },
    {
        ...
        "query": "Throughout the workflow, which activities use data generated by the square_and_quarter activity?", 
        "expected_response": {
    ...
    },
    {
        ...
        "expected_response": {
            "code":"result = df.groupby('workflow_id').agg(first=('started_at','min'), last=('ended_at','max')).reset_index().assign(duration=lambda x: x['last'] - x['first'])[['workflow_id', 'duration']].sort_values(by='duration', ascending=False).head(1)",
    ...
    },
    {
        ...
        "expected_response": {
            "code":"result = df[['workflow_id', 'activity_id']].dropna(axis=1, how='all')",
    ...
    },
    {
        ...
        "expected_response": {
            "code":"""result = df.groupby('workflow_id', as_index=False).agg({"started_at": 'min',"telemetry_summary.cpu.percent_all_diff": "mean"}).sort_values(by='started_at', ascending=False).head(10)[['workflow_id',"telemetry_summary.cpu.percent_all_diff"]]""",
...
llm = build_llm_model(model_name="gpt-4o",temperature=0.01,top_p=1.0)
# L_SCORE PROMPT
...
Return a 0.5 > score >= 0.2 if the generated code tries to answer the user query, shares functionalities or logic comparing with the gold code snippet, but it has issues like different column names or issues like different column names or applying a function to the wrong data type which would lead to an error, even though the error could be easily fixed.

Return a score of 0.0 if the generated code is entirely different from the gold standard code snippet, does not share any meaningful logic or operations, and does not address the user's query.
tandard code snippet, with no meaningful shared aspects, and does not address the user's query.

In all cases, you must give higher scores based on whether the generated code from the AI assistant answers the USER'S QUERY EFFECTIVELY, meaning that the resulting data set is similar to the one generated when running the gold standard code snippet, even if it uses a different approach than the gold standard or if it does not generate an entirely valid Python code, or with different column names, but these errors could be easily fixed.
...
- Minor syntax variations that do not affect functionality.
...
Please provide **only a single float number between 0.0 and 1.0**, feel free to give decimal scores, like 0.78. I want you to give me a score and A SHORT CONCISE reason for said score.
...
for query in queries:
    ...
    try:
        ...
        result = result_wrapper.get("result", None)
        generated_code = (
            result["result_code"]
            if isinstance(result, dict) and "result_code" in result
            else str(result) if result
            else "None"
        )
        # Token & Output fields
        summary_text = result.get("summary", "") if isinstance(result, dict) else ""
        result_df_text = result.get("result_df", "") if isinstance(result, dict) else ""
