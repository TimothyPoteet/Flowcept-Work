...
class Workflow:
    ...
    def run():
        ...
        with Flowcept(workflow_name='hierarchical_math_workflow', start_persistence=False, save_workflow=False):
            ...
            D, C, B = square_and_subtract_one(E), log_and_shift(F), power_one_point_five(G); print(f"E → D: {E} → {D}"); print(f"F → C: {F} → {C}"); print(f"G → B: {G} → {B}")
            A = average_results(D, C, B); print(f"D,C,B → A: ({D}, {C}, {B}) → {A}")
